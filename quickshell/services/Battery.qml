...
Singleton {
    ...
    // A zero percentage means "no reading yet", not "flat". Upstream never has to
    // distinguish the two because UPower only ever publishes a device once it is
    // fully populated; the macOS side polls pmset, so an unpopulated or unreadable
    // sample is reachable. Treating that as flat would fire the automatic-suspend
    // branch below and put the machine to sleep, so every threshold requires a
    // measured value first. A real battery at a true 0% is powering off anyway.
    readonly property bool measured: available && percentage > 0

    property bool isLow: measured && (percentage <= Config.options.battery.low / 100)
    property bool isCritical: measured && (percentage <= Config.options.battery.critical / 100)
    property bool isSuspending: measured && (percentage <= Config.options.battery.suspend / 100)
    ...
    onIsSuspendingAndNotChargingChanged: {
        if (root.available && isSuspendingAndNotCharging) {
            // macOS has no systemd/logind; pmset sleepnow is the unprivileged
            // equivalent of `systemctl suspend`.
            Quickshell.execDetached(["pmset", "sleepnow"]);
        }
    }
