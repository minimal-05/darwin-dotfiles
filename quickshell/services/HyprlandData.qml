...
Singleton {
    ...
    Process {
        id: getMonitors
        command: ["/bin/sh", "-c", `SPACES=$(yabai -m query --spaces); yabai -m query --displays | jq -c --argjson spaces "$SPACES" '[.[] | . as $d | {id: (.index - 1), name: ("Display-" + (.index|tostring)), description: ("Display " + (.index|tostring)), width: (.frame.w|floor), height: (.frame.h|floor), x: (.frame.x|floor), y: (.frame.y|floor), reserved: [0,0,0,0], activeWorkspace: (([$spaces[] | select(.display == $d.index and ."is-visible")] | .[0] | {id: .index, name: (.index|tostring)}) // {id:1,name:"1"}), specialWorkspace: {id: 0, name: ""}, focused: ."has-focus", scale: 1.0, transform: 0, disabled: false}]'`]
        stdout: StdioCollector {
