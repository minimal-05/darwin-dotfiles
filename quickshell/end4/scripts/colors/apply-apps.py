...
SHELL_CONFIG = Path.home() / "Library/Preferences/illogical-impulse/config.json"

# Settings -> Advanced -> Color generation. kitty rides the upstream "Terminal"
# switch; the other two are this fork's, because upstream themes neither.
# enableAppsAndShell is the master and turns all three off.
THEMING_SWITCH = {
...
def theming_enabled(app: str) -> bool:
    """Whether the switch for this app is on.

    Defaults to on for anything the config does not mention: a config written
    before these keys existed should keep behaving as it did, and a missing or
    unreadable config is not a reason to stop theming.
    """
    try:
        theming = json.loads(SHELL_CONFIG.read_text())["appearance"]["wallpaperTheming"]
    except (OSError, ValueError, KeyError):
        return True
    if not theming.get("enableAppsAndShell", True):
        return False
    return bool(theming.get(THEMING_SWITCH[app], True))
...
def apply_firefox(colours: dict, dark: bool, enabled: bool = True) -> str:
    ...
    # Switched off, the stylesheet is emptied rather than left in place. This
    # one is registered live, so an abandoned sheet would keep the browser
    ...
    if not enabled:
        return f"firefox: disabled in settings, cleared {len(profiles)} profile(s) -- {how}"
...
def main() -> int:
    ...
    apps = (
        ("kitty", lambda on: apply_kitty(colours, dark) if on
         else "kitty: disabled in settings, skipped"),
