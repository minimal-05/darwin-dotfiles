    ...
    Variants {
        ...
        LazyLoader {
            ...
            component: PanelWindow { // Bar window
                ...
                // ponytail: the Cocoa backend applies `mask` as a CALayer clip, so unlike
                // Wayland (where it is input-only) anything outside it is never drawn. The
                // hug decorators sit below barContent, so they need masking in or they
                // render as a 2px stub. Corner squares only -- a full-width band under the
                // bar would swallow clicks meant for the windows beneath it.
                readonly property int decoratorSize: roundDecorators.active ? Appearance.rounding.screenRounding : 0
                readonly property int decoratorY: Config.options.bar.bottom ? barContent.y - decoratorSize : barContent.y + barContent.height
                mask: Region {
                    item: hoverMaskRegion

                    Region {
                        x: 0
                        y: barRoot.decoratorY
                        width: barRoot.decoratorSize
                        height: barRoot.decoratorSize
                    }
                    Region {
                        x: barRoot.width - barRoot.decoratorSize
                        y: barRoot.decoratorY
                        width: barRoot.decoratorSize
                        height: barRoot.decoratorSize
                    }
