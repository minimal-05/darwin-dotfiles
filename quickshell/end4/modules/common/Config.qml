    ...
    FileView {
        ...
        JsonAdapter {
            ...
            property JsonObject appearance: JsonObject {
                ...
                // macOS: the corner overlays are hardcoded black and land on the bar, not the
                // wallpaper, so they read as a wedge cut out of it. Off by default here.
                property int fakeScreenRounding: 0 // 0: None | 1: Always | 2: When not fullscreen
                // macOS: end-4's defaults (Google Sans Flex, JetBrains Mono NF,
                // Readex Pro, Space Grotesk) are not installed here, and Qt
                // silently falls back for each one. Roboto Flex is Google Sans
                // Flex's open sibling and carries the same `wdth` axis the
                // clock's width slider drives; it has no `ROND` axis, so the
                // roundness slider stays inert until a font that has one is
                // installed.
                // Upstream ships Google Sans Flex, which is not distributable and is
                // not present here. Helvetica is, on every Mac, which makes it the
                // safe default for a port. Every one of these is editable in
                // Settings -> Interface, and the change now actually takes effect:
                // StyledText used to force Text.NativeRendering, which resolves
                // through CoreText, ignores the requested family and silently
                // substitutes Helvetica -- so whatever was set here, Helvetica is
                // what got drawn. Set these to "Roboto Flex" (installed) for the
                // grotesque end-4 designs against.
                property JsonObject fonts: JsonObject {
                    property string main: "Helvetica"
                    property string numbers: "Helvetica"
                    property string title: "Helvetica"
                    property string iconNerd: "Symbols Nerd Font"
                    property string monospace: "Maple Mono NF"
                    property string reading: "Helvetica"
                    property string expressive: "Helvetica"
                }
            ...
            // macOS: the KDE control modules these defaulted to do not exist.
            // System Settings panes are addressed by their x-apple.systempreferences
            // URL, which `open` resolves.
            ...
            property JsonObject bar: JsonObject {
                ...
                property string topLeftIcon: "spark" // Options: "distro" or any icon name in ~/.config/quickshell/end4/assets/icons
                ...
                property JsonObject workspaces: JsonObject {
                    // Upstream defaults this on, because Linux app icons are flat
                    // SVGs that take a single-colour tint cleanly. macOS icons are
                    // full-colour gradients, and tinting those just yields grey
                    // blobs, so the colour icons read far better here.
            ...
            property JsonObject dock: JsonObject {
                ...
                // ponytail: 2 is upstream's value and relies on Hyprland pinning
            ...
            property JsonObject notifications: JsonObject {
                ...
                property JsonObject monitor: JsonObject {
                    property bool enable: false
                    property string name: "" // Monitor to show notifications on. On macOS this is the display name, e.g. "Built-in Retina Display".
                }
            }
