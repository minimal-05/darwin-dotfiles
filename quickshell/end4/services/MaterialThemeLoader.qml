    ...
    function applyColors(fileContent) {
        ...
        for (const key in json) {
            if (json.hasOwnProperty(key)) {
                ...
                // Generators emit the full Material 3 token set, which is a
                // superset of what Appearance declares. Assigning an undeclared
                // one throws, so skip anything we do not have a slot for.
