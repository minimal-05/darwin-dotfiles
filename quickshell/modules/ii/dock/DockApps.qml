                ...
                RowLayout {
                    ...
                    Repeater {
                        ...
                        RippleButton {
                            ...
                            // ponytail: upstream binds this straight to the
                            // ScreencopyView, which collapses the whole popup to an
                            // empty sliver whenever there is no frame — every time,
                            // back when the macOS shim was inert, and still for a
                            // minimised window now that it is not. The view reports
                            // a zero size until a frame loads, so max() and + are
                            // the whole fallback: a titled row, thumbnail or no.
                            contentItem: ColumnLayout {
                                implicitWidth: Math.max(screencopyView.implicitWidth, root.noPreviewWidth)
                                implicitHeight: root.windowControlsHeight + screencopyView.implicitHeight
