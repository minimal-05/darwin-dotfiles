...
Scope {
    ...
    // The bar is opaque and owns whichever screen edge it sits on. A black
    // quarter-disc drawn on top of it does not read as a rounded display, it
    // scoops a screenRounding-sized bite out of the bar. The display is
    // physically square, so the fake rounding only earns its place on the edges
    // showing wallpaper and windows.
    //
    // Float is the exception: it insets the bar by hyprlandGapsOut, so the
    // corner pixel is wallpaper again and the disc belongs there. The wedge
    // stays clear of the pill -- at the pill's 8px inset it is only 5.6px deep.
    ...
    readonly property bool barOwnsEdge: Config.options.bar.cornerStyle !== 1 // 1: Float
    ...
    component CornerPanelWindow: PanelWindow {
        ...
        visible: !(cornerPanelWindow.coveredByBar && screenCorners.barOwnsEdge)
